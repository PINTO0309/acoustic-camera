...
from bokeh.models import Div, CheckboxGroup, RadioButtonGroup, TextInput, Button, ColumnDataSource, Spacer, Select #type:ignore
...
class Dashboard:
    def __init__(self, config, processor, model_on=False, alphas=None, video_stream=None, stream_on=False):
        ...
        self._sync_beamforming_view(self.config.get('app_default_settings.z'))
        ...
        if processor.dev is not None:
            ...
            self.peak_marker_fov_options = ["90", "100", "120", "180"]
            peak_marker_fov = str(self.config.get('app_settings.peak_marker_fov', 90))
            if peak_marker_fov not in self.peak_marker_fov_options:
                peak_marker_fov = "90"

            # Frequency input field
            self.f_input = TextInput(value=str(self.processor.frequency), title="Frequency (Hz)", width=inside_width)
            self.peak_marker_fov_select = Select(
                value=peak_marker_fov,
                title="Peak Marker FOV",
                options=self.peak_marker_fov_options,
                width=inside_width,
            )
            self.acoustic_camera_plot.update_peak_marker_fov(int(self.peak_marker_fov_select.value))
    ...
    def setup_layout(self):
        ...
        # Checkboxes for optional overlays
        self.microphone_checkboxes = CheckboxGroup(
            labels=["Show Microphone Geometry", "Show Origin", "Show Heatmap Overlay"],
            active=[],
        )
    ...
    def setup_callbacks(self):
        ...
        self.peak_marker_fov_select.on_change("value", self.update_peak_marker_fov)
        ...
        if self.processor.dev is not None:
            self.toggle_visibility("active", None, self.microphone_checkboxes.active)
    ...
    def update_real_z(self, attr, old, new):
        ...
        try:
            z = float(new)
            self.real_z = z
            self._sync_beamforming_view(z)
        except ValueError:
            pass

    def _sync_beamforming_view(self, z):
        self.acoustic_camera_plot.update_view_range(z)
        self.acoustic_camera_plot.update_beamforming_extent(
            self.acoustic_camera_plot.xmin,
            self.acoustic_camera_plot.xmax,
            self.acoustic_camera_plot.ymin,
            self.acoustic_camera_plot.ymax,
        )
        self.processor.update_beamforming_extent(
            self.acoustic_camera_plot.xmin,
            self.acoustic_camera_plot.xmax,
            self.acoustic_camera_plot.ymin,
            self.acoustic_camera_plot.ymax,
    ...
    def update_peak_marker_fov(self, attr, old, new):
        try:
            self.acoustic_camera_plot.update_peak_marker_fov(int(new))
        except ValueError:
            pass
    ...
    def toggle_visibility(self, attr, old, new):
        ...
        self.acoustic_camera_plot.toggle_beamforming_overlay(2 in new)
